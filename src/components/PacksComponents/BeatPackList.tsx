...
const containerVariants = {
  ...
  visible: {
    ...
    translateY: 0,
    transition: {
      staggerChildren: 1.25, // Delay between items
      delayChildren: 1.25,
      duration: 0.9,
    },
  },
...
const BeatPackList = ({ isHomePage = false }: { isHomePage?: boolean }) => {
  const [
    searchParams,
    // setSearchParams
  ] = useSearchParams();
  ...
  return (
    <section className={`${isHomePage ? 'py-4' : 'py-16'} z-50 relative`}>
      <div className="container mx-auto flex flex-col items-center gap-16 lg:px-16">
        <div className="text-center">
          ...
          <p className="mb-8 text-muted-foreground md:text-base lg:max-w-2xl lg:text-lg">
            Industry-ready beat packs crafted for hip hop, trap, and West Coast
            artists. Exclusive sounds, royalty-free, and built to inspire.
          </p>
        ...
        >
          {!isPacksLoaded || isFetching
            ? Array.from({ length: 3 }).map((_, index) => (
                <Skeleton key={index} className="!h-[500px] w-[500px]" />
              ))
            : packs
                .map((pack) => {
                  ...
                  return (
                    ...
                    >
                      <Card
                        key={pack.id}
                        //   className="grid grid-rows-[auto_auto_1fr_auto] pt-0 overflow-hidden bg-background/10"
                        className="pt-0 max-w-md overflow-hidden bg-background/10"
                      >
                        <button
                          onClick={(e) => handleTrackPlay(pack, e)}
                          className="aspect-square  relative overflow-hidden !p-0 !m-0"
                        >
                        ...
                        <CardHeader>
                          ...
                          <Tooltip>
                            <TooltipTrigger
                              onClick={() =>
                                navigate(`/beat-pack?packId=${pack.id}`)
                              }
                            >
                ...
                .slice(isHomePage ? 0 : undefined, isHomePage ? 3 : undefined)}
        </motion.div>
        {isHomePage && (
          <NavLink to="/beat-packs" className="z-50">
            <button
              className={`w-fit !text-white hover:!bg-white hover:!text-black  !transition-all !duration-600 !bg-zinc-900`}
            >
              Browse All Beat Packs
            </button>
          </NavLink>
        )}
      </div>
